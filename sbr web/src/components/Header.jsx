        ...
        <header className="w-full fixed top-0 left-0 z-50">
                ...
                <nav className="container mx-auto px-6 py-4 flex justify-between items-center">
                    ...
                    <ul className="hidden md:flex flex-grow justify-center items-center space-x-1">
                        {navLinks.map(item => {
                                                ...
                                                <div className="p-6 border-r border-gray-200">
                                                    ...
                                                    <ul className="space-y-2">
                                                        {productLinks.map(link => (
                                                            <li key={link.name}>
                                                                <a href="#" onClick={(e) => { e.preventDefault(); handleNavigation(link.route); }} className="block text-brand-dark-blue font-medium hover:text-brand-blue hover:translate-x-1 transition-all">
                                                                    {link.name}
                                                                </a>
                                                            </li>
                                                        ))}
                            ...
                            return (
                                <li key={item}>
                                    <a href="#" onClick={(e) => { e.preventDefault(); handleNavigation(item.toLowerCase()); }} className={`font-semibold transition-all duration-300 transform active:scale-95 px-4 py-2 rounded-md ${isScrolled ? 'text-white hover:bg-brand-yellow hover:text-brand-dark-blue' : 'text-brand-dark-blue hover:bg-brand-yellow/20'}`}>
                                        {item}
                                    </a>
                                </li>
                            );
                        ...
                        {user ? (
                            <li>
                                <a href="#" onClick={(e) => { e.preventDefault(); handleNavigation(`${user.role.toLowerCase()}-dashboard`); }} className={`font-semibold transition-all duration-300 transform active:scale-95 px-4 py-2 rounded-md flex items-center gap-1.5 ${isScrolled ? 'text-white hover:bg-brand-yellow hover:text-brand-dark-blue' : 'text-brand-dark-blue hover:bg-brand-yellow/20'}`}>
                                    Dashboard
                                </a>
                            </li>
                        ) : (
                            <li>
                                <a href="#" onClick={(e) => { e.preventDefault(); handleNavigation('auth'); }} className={`font-semibold transition-all duration-300 transform active:scale-95 px-4 py-2 rounded-md flex items-center gap-1.5 ${isScrolled ? 'text-white hover:bg-brand-yellow hover:text-brand-dark-blue' : 'text-brand-dark-blue hover:bg-brand-yellow/20'}`}>
                    ...
                    <div className="hidden md:flex items-center space-x-3">
                        {user ? (
                            <a href="#" onClick={(e) => { e.preventDefault(); handleNavigation(`${user.role.toLowerCase()}-dashboard`); }} className={`font-bold py-2 px-5 rounded-full flex items-center gap-2 transition-all duration-300 ${isScrolled ? 'bg-brand-yellow text-brand-dark-blue hover:bg-white' : 'bg-brand-blue text-white hover:bg-brand-dark-blue'}`}>
                                <IconUser className="h-4 w-4" /> Dashboard
                            </a>
                        ) : (
                            <a href="#" onClick={(e) => { e.preventDefault(); handleNavigation('auth'); }} className={`font-bold py-2 px-5 rounded-full flex items-center gap-2 transition-all duration-300 ${isScrolled ? 'bg-brand-yellow text-brand-dark-blue hover:bg-white' : 'bg-brand-blue text-white hover:bg-brand-dark-blue'}`}>
                                <IconUser className="h-4 w-4" /> Login
                            </a>
                        )}
                        <a href="#" onClick={(e) => { e.preventDefault(); openContactModal(); }} className={`font-bold py-2 px-5 rounded-full transition-all duration-300 ${isScrolled ? 'bg-white text-brand-dark-blue hover:bg-brand-yellow' : 'bg-brand-blue text-white hover:bg-brand-dark-blue'}`}>
                            Contact Us
                        </a>
                    </div>

                    {/* Mobile menu and contact/login buttons */}
                    <div className="md:hidden flex items-center space-x-2">
                        {user ? (
                            <button onClick={() => handleNavigation(`${user.role.toLowerCase()}-dashboard`)} className={`p-2 rounded-full transition-all duration-300 ${isScrolled ? 'bg-white text-brand-dark-blue hover:bg-brand-yellow' : 'bg-brand-blue text-white hover:bg-brand-dark-blue'}`} aria-label="Dashboard" title="Dashboard">
                                <IconUser className="h-5 w-5" />
                            </button>
                        ) : (
                            <button onClick={() => handleNavigation('auth')} className={`p-2 rounded-full transition-all duration-300 ${isScrolled ? 'bg-white text-brand-dark-blue hover:bg-brand-yellow' : 'bg-brand-blue text-white hover:bg-brand-dark-blue'}`} aria-label="Login" title="Login">
                                <IconUser className="h-5 w-5" />
                            </button>
                        )}
                        ...
                        <button onClick={() => setNavOpen(!navOpen)} className={`ml-2 ${isScrolled ? 'text-white' : 'text-brand-dark-blue'}`} aria-label="Toggle menu">
            ...
            <div className={`md:hidden absolute top-full left-0 w-full bg-brand-dark-blue shadow-lg transition-all duration-300 ease-in-out ${navOpen ? 'max-h-screen' : 'max-h-0 overflow-hidden'}`}>
                <ul className="flex flex-col p-4">
                    {navLinks.map(item => (
                        <li key={item}>
                            <a href="#" onClick={(e) => { e.preventDefault(); closeNav(); handleNavigation(item.toLowerCase()); }} className="block py-3 px-4 text-white font-medium hover:bg-brand-blue rounded-md active:bg-brand-blue">
                                {item}
                            </a>
                        </li>
                    ))}
                    {user ? (
                        <li key="portal-mobile">
                            <a href="#" onClick={(e) => { e.preventDefault(); closeNav(); handleNavigation(`${user.role.toLowerCase()}-dashboard`); }} className="block py-3 px-4 text-white font-medium hover:bg-brand-blue rounded-md active:bg-brand-blue flex items-center gap-2">
                                <IconUser className="w-5 h-5" /> Dashboard
                            </a>
                        </li>
                    ) : (
                        <li key="login-mobile">
                            <a href="#" onClick={(e) => { e.preventDefault(); closeNav(); handleNavigation('auth'); }} className="block py-3 px-4 text-white font-medium hover:bg-brand-blue rounded-md active:bg-brand-blue flex items-center gap-2">
                                <IconUser className="w-5 h-5" /> Login / Account
                            </a>
                        </li>
                    )}
                </ul>
